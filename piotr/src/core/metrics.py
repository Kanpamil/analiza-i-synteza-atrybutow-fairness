...
class FairReport(Report):
    ...
    equalized_odds_difference_mean = metric(lambda self: (np.abs(self.prot.tpr - self.unprot.tpr) + np.abs(self.prot.fpr - self.unprot.fpr)) / 2)
    equalized_odds_difference_max = metric(lambda self: np.maximum(np.abs(self.prot.tpr - self.unprot.tpr), np.abs(self.prot.fpr - self.unprot.fpr)))
    equalized_odds_ratio_mean = metric(lambda self: (safe_div(self.prot.tpr, self.unprot.tpr) + safe_div(self.prot.fpr, self.unprot.fpr)) / 2)
    equalized_odds_ratio_min = metric(lambda self: np.minimum(safe_div(self.prot.tpr, self.unprot.tpr), safe_div(self.prot.fpr, self.unprot.fpr)))
